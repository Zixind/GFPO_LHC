...
def _collect_state(agent: Any) -> Dict[str, Dict[str, Any]]:
    ...
    Returns dict with keys: "modules", "optimizers".
    """
    seen = set()
...
def save_agent_mc(agent: Any, out_pt: Path, *, meta: Optional[dict] = None):
    ...
    bundle = {
        ...
        "state": state,              # {"modules":..., "optimizers":...}
        "meta": meta or {},
    }
    torch.save(bundle, str(out_pt))

    # also write a small human-readable meta json
...
def save_registry_mc(registry: dict, out_dir: Path, *, meta_common: Optional[dict] = None):
    """
    registry format (what you already have):
      {
        "AD": {"DQN": agent_obj, "GRPO": agent_obj, "GFPO-F": agent_obj, ...},
        "HT": {"DQN": agent_obj, ...},
      }
    """
...
def write_paper_table(rows, out_csv: Path, out_tex: Path, target_pct, tol_pct):
    ...
    # Force output order
    trigger_order = ["HT", "AD"]
    method_order  = ["Constant", "PID", "ADT", "DQN", "DQN_GAMMA_0", "PPO", "GRPO", "GFPO-F", "GFPO-FR"]
...
def save_all_models(models_dir: Path, tag: str, agents: dict, meta_common: dict):
    """
    agents: dict name -> agent_obj
    """
    for name, agent in agents.items():
        path_pt = models_dir / tag / f"{name}.pt"
        save_agent_bundle(agent, path_pt, meta=meta_common | {"model_name": name})
...
def main():
    ...
    ap.add_argument("--models-dir", type=str, default="outputs/models")
    ...
    dqn_as_gamma_0 = SeqDQNAgent(seq_len=K, feat_dim=feat_dim_as, n_actions=len(AS_DELTAS),
                        cfg=dqn_cfg_gamma_0, seed=SEED, lambda_1 = args.lambda_1)
    ...
    AGENT_REGISTRY = {
        "AD": {},   # AD / AS trigger agents
        "HT": {},   # HT trigger agents
    }

    # after creating agents:
    AGENT_REGISTRY["AD"]["DQN"]     = dqn_as
    AGENT_REGISTRY["AD"]["ADT"]     = agent_adt_as   # ADT uses same DQN weights (action-hold is in controller)
    AGENT_REGISTRY["AD"]["PPO"]     = ppo_agent_ad
    ...
    if args.run_ht:
        ...
        AGENT_REGISTRY["HT"]["DQN"]     = dqn_ht
        AGENT_REGISTRY["HT"]["ADT"]     = agent_adt_ht
        AGENT_REGISTRY["HT"]["PPO"]     = ppo_agent_ht
    ...
    if args.save_models:
        save_registry_mc(
            AGENT_REGISTRY,
            out_dir=Path(args.outdir) / "models_mc",
            meta_common={
                "seed": SEED,
                "run_label": run_label,
                "as_dim": args.as_dim,
                "target": args.target,
                "tol": args.tol,
                "mc_file": "Data/Trigger_food_MC.h5",
            },
        )

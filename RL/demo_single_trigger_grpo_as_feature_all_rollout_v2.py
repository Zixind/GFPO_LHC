...
#!/usr/bin/env python3
"""
demo_single_trigger_grpo_as_feature_all_training_mc.py

Single-trigger threshold control with event-sequence features with sliding window. Train on MC and Roll out on real data.
...
class DQNCtrl(BaseCtrl):
    def __init__(self, name, init_cut, lo, hi, *, agent, deltas, step, max_delta, as_mid, as_span,
                 near_widths, K, target, tol, eps_min, eps_decay, train_steps_per_micro, alpha, beta, train:bool = True):
        self.name = name
        self.cut = float(init_cut)
        self.lo, self.hi = float(lo), float(hi)
        self.agent = agent
    ...
    def step_micro(self, *, bas_w, bnpv_w, bas_j, sas_tt, sas_aa, micro_global,
        ...
        eps = 0.0 if not self.train else max(self.eps_min, 1.0*(self.eps_decay**self.step_count))
...
class DQNCtrlHT(BaseCtrl):
    def __init__(self, name, init_cut, lo, hi, *, agent, deltas, step, max_delta,
                 ...
                 train_steps_per_micro, alpha, beta, train: bool = True):
        self.name = name
        self.cut = float(init_cut)
        self.lo, self.hi = float(lo), float(hi)
        self.agent = agent
...
class ADTCtrl(DQNCtrl):
    def __init__(self, *args, adt_l=10, train_steps_per_episode=50,
                 reward_mode="lhc", adt_alpha=0.7, adt_beta=0.3, train: bool = True, **kwargs):
...
class PPOCtrl(BaseCtrl):
    ...
    def end_chunk(self, *, chunk: int, **kwargs):
        # bootstrap at end-of-chunk and not self.train we would skip this
        if not self.train:
            return
        last_v = 0.0
...
class GRPOCtrl(BaseCtrl):
    ...
    """
    def __init__(self, name, init_cut, lo, hi, *, agent, deltas, step, max_delta, as_mid, as_span,
                 near_widths, K, target, tol, train_every, temperature, group_size_keep: int, train: bool = True):
        self.name = name
        self.cut = float(init_cut)
        self.lo, self.hi = float(lo), float(hi)
        self.agent = agent
    ...
    def step_micro(self, *, chunk: int, bas_w, bnpv_w, bas_j, sas_tt, sas_aa, micro_global, grpo_samples=None):
        ...
        if (self.micro_counter % self.train_every) == 0 and self.train:
            _ = self.agent.update()
...
class GFPOCtrl(GRPOCtrl):
    ...
    def __init__(self, *args, gfpo_filter="abs_err_topk", group_size_sample=32, group_size_keep=16,
                 feas_mult=1.0, mix=0.80, band_mult=1.0, sig_bonus=1.0, train:bool = True, **kwargs):
        super().__init__(*args, group_size_keep=group_size_keep, **kwargs)
...
class GRPOCtrlHT(BaseCtrl):
    ...
    """
    def __init__(self, name, init_cut, lo, hi, *, agent, deltas, step, max_delta,
                 ht_mid, ht_span, near_widths, K, target, tol, train_every, temperature, group_size_keep: int, train: bool = True):
        self.name = name
        self.cut = float(init_cut)
        self.lo, self.hi = float(lo), float(hi)
        self.agent = agent
...
def main():
    ...
    ap.add_argument("--input", default="Data/Matched_data_2016_dim2.h5", #Data/Trigger_food_MC.h5
                    choices=["Data/Trigger_food_MC.h5", "Data/Matched_data_2016_dim2.h5", "Data/Trigger_food_MC_ablation_4.h5", "Data/Trigger_food_MC_ablation_6.h5", "Data/Trigger_food_MC_ablation_8.h5", "Data/Matched_data_2016_with04_paper.h5"])
    ...
    agent = GRPOAgent(seq_len=K, feat_dim=feat_dim_as, n_actions=len(AS_DELTAS), cfg=cfg, seed=SEED, #This is GRPO
    ...
    if "gfpo_fr" in BASELINES:
        gfpo_fr_as = GRPOAgent(seq_len=K, feat_dim=feat_dim_as, n_actions=len(AS_DELTAS), cfg=cfg, seed=SEED,
                        reward_cfg=agent.reward_cfg)
        controllers_as.append(GFPOCtrl(

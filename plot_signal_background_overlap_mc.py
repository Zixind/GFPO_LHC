#!/usr/bin/env python3
"""
Signal-background overlap figure for MC (Trigger_food_MC.h5).
...
Usage:
    conda run -n adaptive python plot_signal_background_overlap_mc.py
"""
...
OUT        = Path("outputs/signal_background_overlap")
H5         = Path("Data/Trigger_food_MC.h5")
CHUNK_SIZE = 50_000
START      = CHUNK_SIZE * 10   # = 500_000
...
e_ht_tt_oracle, e_ht_aa_oracle = read_oracle_avg("chunk_cuts_ht2_99725_mc.csv")
e_as_tt_oracle, e_as_aa_oracle = read_oracle_avg("chunk_cuts_as2_99725_mc.csv")
...
Bnpv = np.asarray(d["Bnpv"], dtype=np.float64) if "Bnpv" in d else None
Tnpv = np.asarray(d["Tnpv"], dtype=np.float64) if "Tnpv" in d else None
Anpv = np.asarray(d["Anpv"], dtype=np.float64) if "Anpv" in d else None

bht_eval  = Bht[START:]
bas_eval  = Bas[START:]
bnpv_eval = Bnpv[START:] if Bnpv is not None else None

print(f"Background events from event {START}: {len(bht_eval):,}")

# ── per-chunk ROC for smooth curve ───────────────────────────────────────────
def roc_curve_perchunk(bg_all, sig_all, sig_npv, bnpv_all, chunk_size=CHUNK_SIZE,
                       n_pts=400):
    """Per-chunk-average (r_bg[%], eff_sig[%]) for a sweep of single cuts.

    Gives a smooth ROC showing discriminability of the fixed-cut strategy.
    Oracle CSV values (per-chunk adaptive cuts) are annotated separately.
    """
    ...
    while start < n:
        ...
        if bnpv_all is not None and sig_npv is not None:
            bnpv_c = bnpv_all[start:end]
            if len(bnpv_c) > 0:
                npv_min = float(np.min(bnpv_c))
                npv_max = float(np.max(bnpv_c))
                sig_c = sig_all[(sig_npv >= npv_min) & (sig_npv <= npv_max)]
            else:
                sig_c = sig_all
        else:
            sig_c = sig_all
...
r_ht_tt, eff_ht_tt, _ = roc_curve_perchunk(bht_eval, Tht, Tnpv, bnpv_eval)
r_ht_aa, eff_ht_aa, _ = roc_curve_perchunk(bht_eval, Aht, Anpv, bnpv_eval)
r_as_tt, eff_as_tt, _ = roc_curve_perchunk(bas_eval, Tas, Tnpv, bnpv_eval)
r_as_aa, eff_as_aa, _ = roc_curve_perchunk(bas_eval, Aas, Anpv, bnpv_eval)
...
cut_ht = oracle_cut_global(bht_eval, R_PLUS)
cut_as = oracle_cut_global(bas_eval, R_PLUS)

# ── histogram signal pools ────────────────────────────────────────────────────
def npv_union_pool(bg_all, bnpv_all, sig_all, sig_npv, chunk_size=CHUNK_SIZE):
    if bnpv_all is None or sig_npv is None:
        return sig_all[np.isfinite(sig_all)]
    n = len(bg_all)
    mask = np.zeros(len(sig_all), dtype=bool)
    start = 0
    while start < n:
        end = min(start + chunk_size, n)
        bnpv_c = bnpv_all[start:end]
        if len(bnpv_c) > 0:
            mask |= (sig_npv >= float(np.min(bnpv_c))) & \
                    (sig_npv <= float(np.max(bnpv_c)))
        start += chunk_size
    return sig_all[mask]

tht_pool = npv_union_pool(bht_eval, bnpv_eval, Tht, Tnpv)
aht_pool = npv_union_pool(bht_eval, bnpv_eval, Aht, Anpv)
tas_pool = npv_union_pool(bas_eval, bnpv_eval, Tas, Tnpv)
aas_pool = npv_union_pool(bas_eval, bnpv_eval, Aas, Anpv)
...
def plot_score_hist(ax, bg, sig_aa, sig_tt, cut, title, xlabel):
    ...
    ax.hist(bg[np.isfinite(bg)], color=C_BG, label="Background (MC)", **kw)
...
plot_score_hist(ax_ht, bht_eval, aht_pool, tht_pool, cut_ht,
                r"(a) $H_T$ score", r"$H_T$ [GeV]")
plot_score_hist(ax_as, bas_eval, aas_pool, tas_pool, cut_as,
